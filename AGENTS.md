...
1. **Не останавливать и не включать cron-джобы и не менять расписание** без
   команды владельца — исключение: аварийная пауза Publisher при явном
   инциденте (см. «Авария»).
2. **Не менять** `autopilot_paused` в `config/policy.yaml` и секцию `safety:`.
3. **Не менять состояние монетизации** без команды владельца: трекинг
   (`TRACKING_BASE_URL`, systemd-юнит `aibp-redirect`) и
   `telegram.source_button` включены с 2026-07-20 (этап 8) — не выключать
   и не переконфигурировать самостоятельно.
...
- **Этап:** ПРОД (cutover 2026-07-14) — ЭТА система единственный издатель
  @AI_Business_Pulse. Старая (n8n + Hermes cloud) выключена и осталась
  только как аварийный резерв.
- Активны 10 джоб: collect/enrich/publish/engagement/dashboard +
  прод-генерации (Morning/Weekly Case/Evening/Weekly Digest, дайджест —
  Вс 18:00, публикация 19:00) + Safety Check.
  Stage-генерации ВЫКЛЮЧЕНЫ с 2026-07-16 (экономия LLM; тест-канал
  включается точечно для QA рискованных изменений и вернётся на этапе 9
  для shadow-экспериментов). Также на паузе до этапа 9: miner, policy
  updater, shadow runner, decision engine, rollback check, approval gate.
  `autopilot_paused: true`.
- **Монетизация (этап 8, с 2026-07-20):** трекинг кликов включён —
  redirect-сервис как systemd-юнит `aibp-redirect` (порт 8091, публично
  `https://aibp.borovikvv.ru/r/...` через nginx), Redirect Health Check
  крон активен, `telegram.source_button: true`. Каталог офферов пуст —
  добавляет только владелец (`aibp offer-add`).
- Картинки: у morning/weekly_case включены в проде с 2026-07-14;
  вечерняя заметка всегда текстовая (`visual_policy.slots`).
...
ВАЖНО: резервная (старая) система живёт на ДРУГИХ серверах
(n8n.borovikvv.ru + Hermes cloud) — у тебя к ним доступа НЕТ, включить её
можешь только владелец. Твой предел в аварии: остановить публикацию здесь
и эскалировать. Канал без постов лучше канала с плохими постами.

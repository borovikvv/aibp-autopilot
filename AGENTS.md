...
3. **Не включать монетизацию в проде**: в `config/policy.yaml`
   `visual_policy.enabled/generate`, `telegram.source_button`,
   `TRACKING_BASE_URL` (на stage картинки уже включены — это норма).
...
- Активны 10 джоб: collect/enrich/publish/engagement/dashboard +
  ...
  redirect health — до монетизации. `autopilot_paused: true`.
- Картинки: у morning/weekly_case (stage — включено, прод — до этапа 8);

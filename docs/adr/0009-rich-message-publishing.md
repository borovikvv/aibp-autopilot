...
- **Авто-рендер схемы** по `visual_policy.kind` — ~~в репозитории нет генератора~~
  реализован в #34: `aibp.generation.image_gen` через OpenRouter
  `/api/v1/images` (`google/gemini-2.5-flash-image`). PNG сохраняется в
  `IMAGE_OUTPUT_DIR` и отдаётся публичным URL, который этот publish-путь и
  потребляет. Включается `visual_policy.enabled` + `visual_policy.generate`.
- **Видео** (`sendVideo`) — генерация видео отсутствует; ветку добавим, когда
  будет источник.
